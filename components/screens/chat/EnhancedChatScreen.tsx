...
const EnhancedChatScreen: React.FC<EnhancedChatScreenProps> = ({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    ...
    },
    {
      ...
      message: 'It\'s been a while since your last health assessment. Would you like to do a quick checkup?',
      tipType: 'suggestion',
      priority: 'medium',
      actionButton: {
  ...
  const handleSymptomCheckerAnswer = useCallback((questionId: string, answer: string) => {
  ...
  const handleMedicationComplete = useCallback((reminderId: string) => {
    setMessages(prev => prev.map(msg => {
      if (msg.type === 'medication-reminder' && msg.reminderId === reminderId) {
  ...
  const handleGoalProgressUpdate = useCallback((goalId: string, newProgress: number) => {
    setMessages(prev => prev.map(msg => {
  ...
  const handleProactiveAction = useCallback((action: string) => {
  ...
  const renderMessage = useCallback(({ item }: { item: ChatMessage }) => {
    const animation = messageAnimations.current.get(item.id) || createMessageAnimation(item.id);
  ...
  const quickActions = [
    { id: 'health-check', title: 'Health Check', icon: '🔍', action: 'start-assessment' },
    { id: 'medication', title: 'Medication', icon: '💊', action: 'medication-reminder' },
    { id: 'goals', title: 'Goals', icon: '🎯', action: 'goal-tracking' },
    { id: 'education', title: 'Learn', icon: '📚', action: 'educational-content' },
  ];
    ...
    switch (action) {
      ...
      case 'educational-content': {
        const educationMessage: ChatMessage = {
          ...
          content: 'Staying hydrated is crucial for maintaining optimal health. Water helps regulate body temperature, supports digestion, and keeps your skin healthy.',
          category: 'nutrition',
          quiz: {
            question: 'How much water should an average adult drink daily?',
            options: ['6-8 glasses', '10-12 glasses', '4-6 glasses', '12-14 glasses'],
            correctAnswer: 0,
          },
          isUser: false,
          timestamp: new Date(),
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {messages.length <= 2 && !isLoading && (
          <View style={[styles.quickActionsContainer, { backgroundColor: colors.surface, borderTopColor: colors.background }]}>
            ...
            <View style={styles.quickActionsGrid}>
              {quickActions.map((action) => (
                ...
                >
                  ...
                  <Text style={[styles.quickActionText, { color: colors.primary }]}>{action.title}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
